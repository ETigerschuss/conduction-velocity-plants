...
kids.push(p([b("From near to far the potential attenuates and broadens, while largely preserving its shape "), t("(far/near amplitude median ≈ 0.43, FWHM ratio ≈ 1.30, waveform correlation ≈ 0.80). Within-species spread exceeds between-species differences.")]));
kids.push(...splitfig("self_aligned_channels.png", "Near vs far response per species, each self-aligned on its own peak (arbitrary gap), both normalised to the near peak so the far amplitude drop is visible. Thin traces coloured by waveform duration (short-duration green, long-duration purple); thick = median, band = IQR; far/near ratio printed per panel."));
kids.push(...splitfig("near_to_far_transformation.png", "The three metrics pooled across all recordings: amplitude ratio (median 0.43), width ratio (1.30), waveform similarity (0.80)."));
kids.push(...splitfig("attenuation_vs_broadening.png", "Attenuation vs broadening co-vary: recordings that lose amplitude also disperse, consistent with low-pass filtering along the path. Note the amplitude axis is confounded by electrode coupling (Section 6), so this co-variation is descriptive and not evidence of propagation mode."));
...
kids.push(...splitfig("delay_vs_distance.png", "Delay vs distance: linear through ~origin (exponent ≈ 1.09, 95 % CI ± 0.37) — consistent with constant-velocity propagation and inconsistent with diffusion (which would give ≈ 2)."));
...
kids.push(p([b("Solanaceae are not internally more similar. "), t("A Mantel test between functional distance and taxonomic distance is null (r = −0.02, p = 0.50). Breaking it down by family: Lamiaceae (5 species) are marginally more similar within (mean within-distance 2.35 vs 2.89 to others), but "), b("Solanaceae (3 species) are not"), t(" — the two Capsicum chiles and Tomato are more different from each other (3.72) than from species in other families (3.00). Congeners diverge as much as unrelated species. The propagation profile reflects the biophysics and waveform of each conduction event, not the species’ ancestry.")]));
...
kids.push(p([b("Headline figures — the defensible results: "), t("(1) CV by species (Fig. 1) — the comparative dataset, thirteen species on low-cost two-channel hardware; (2) validation against the manual analysis (Fig. 2) — the automatic pipeline reproduces hand measurement at r = 0.94; (3) the near→far transformation (Fig. 10) — attenuation ≈ 0.43, broadening ≈ 1.30, shape preserved ≈ 0.80; (4) the all-parameter clustering / phylogeny null (Fig. 34) — propagation profile is not phylogenetically structured, and Solanaceae are not internally more similar.")]));
